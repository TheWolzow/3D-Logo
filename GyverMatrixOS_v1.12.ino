...
// '3d', 16x16px
const unsigned char logo_16x163d [] PROGMEM = {
	0x01, 0x00, 0x3f, 0xc0, 0x0f, 0xf8, 0x03, 0xf0, 0xc0, 0x00, 0x70, 0x1e, 0x1c, 0xff, 0x0c, 0xe1, 
	0x0c, 0x81, 0x38, 0x81, 0x0c, 0x81, 0x04, 0x81, 0xc4, 0x83, 0x74, 0xfe, 0x1c, 0xfc, 0x00, 0x00
};

bool getPixelFromBitmap(const uint8_t *bitmap, int x, int y) {
  if (x < 0 || x >= 16 || y < 0 || y >= 16) return false;

  uint16_t i = (y * 16 + x) / 8;
  uint8_t b = pgm_read_byte(&bitmap[i]);
  return b & (1 << (7 - ((y * 16 + x) % 8)));
}

void drawBitmap3D(const uint8_t *bitmap, float angle, CRGB frontColor, CRGB backColor, CRGB outlineColor, int pivotX = 8) {
  float scale = cos(angle);
  float skew  = sin(angle);
  float absScale = abs(scale);

  bool frontSide = scale >= 0;
  float brightness = pow((absScale + 1.0) * 0.5, 1.3);
  brightness = max(brightness, 0.2);

  for (int y = 0; y < 16; y++) {
    for (int x = 0; x < 16; x++) {

      bool pixel = getPixelFromBitmap(bitmap, x, y);

      // проверка соседей для контура
      bool hasNeighbor =
        getPixelFromBitmap(bitmap, x+1, y) ||
        getPixelFromBitmap(bitmap, x-1, y) ||
        getPixelFromBitmap(bitmap, x, y+1) ||
        getPixelFromBitmap(bitmap, x, y-1);

      if (!pixel && !hasNeighbor) continue;

      float xCentered = x - pivotX;

      float dir = (cos(angle) >= 0) ? 1.0 : -1.0;
      float xRot = xCentered * scale + skew * 1.0 * dir;
      int newX = round(xRot + pivotX);

      if (!frontSide) {
        newX -= 1;
      }
      //Serial.println(String(xRot) + " + " + String(pivotX) + " = " + String(round(xRot + pivotX)));
      ...
      if (newX >= 0 && newX < 16) {
        ...
        if (pixel) {
          color = frontSide ? frontColor : backColor;
        } else {
          color = outlineColor;  // контур
        }

        uint8_t b = constrain(brightness * 255, 50, 255);
        color.nscale8(b);

        leds[getPixelNumber(newX, 15 - y)] = color;
      }
    }
  }
}
...
void loop() {
  ...
  if (!spinning) {
    // стоим на месте (0 градусов)
    angle = 0;

    if (now - timer > 3000) {   // 1.5 секунды пауза
      spinning = true;
      timer = now;
    }
    ...
    // быстро крутимся
    angle += 0.1;   // скорость вращения (быстрее чем было)
  ...
  drawBitmap3D(
    logo_16x163d,
    angle,
    CRGB(255,255,255),   // front
    CRGB(80,80,255),      // back (синий оттенок)
    CRGB(0, 0, 10),
    8
  );
